...
export async function createNodeFromModal({
  ...
  switch (type) {
    case 'folder':
      ...
      break
    case 'image':
      const imageData = await getImageMetadata(content)
      newNode = {
        content: content,
        dateCreated: new Date(),
        filePath: filePath,
        ...
        title: title,
        type: type,
        originalHeight: imageData.normalizedHeight,        
        originalWidth: imageData.normalizedWidth,
        newHeight: imageData.normalizedHeight,
        newWidth: imageData.normalizedWidth,
      }
      break
    case 'temporal':
      newNode = {
