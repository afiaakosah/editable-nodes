...
import { ILink, INode, NodeIdsToNodesMap } from '../../../types'
...
/**
 * Modal for completing a link; lets the user choose a title and 
 * explainer.
 */
export const CompleteLinkModal = (props: ICompleteLinkModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    if (startAnchor && endAnchor) {
      ...
      if (link !== null) {
        handleClose()
        const startNodeResp = await FrontendNodeGateway.getNode(startAnchor.nodeId)
        setSelectedNode(startNodeResp.payload)
        setIsLinking(false)
        setStartAnchor(null)
        setRefresh(!refresh)
      } else {
