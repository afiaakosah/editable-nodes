...
import { linkedTimestamp } from '../../../global/Atoms'
...
interface ILinkItemProps {
  ...
  setLinkedTimestamp: (timestamp: number | null) => void
}
...
export const fetchNodeFromLink = async (props: ILinkItemProps) => {
  ...
  // the timestamp on a temporal node to which we will seek after a link is clicked
  // const setLinkedTimestamp = useSetRecoilState(linkedTimestamp)
  ...
  if (firstAnchor.payload !== null && secondAnchor.payload !== null) {
    ...
    if (
      firstNode?.nodeId === currentNode?.nodeId &&
      secondNode?.nodeId &&
      secondAnchor.payload.extent
    ) {
      setSelectedNode(secondNode)
      if (secondAnchor.payload.extent.type === 'temporal') {
        setLinkedTimestamp(secondAnchor.payload.extent.startTimestamp)
      }
      setRefresh(!refresh)
      return secondNode.nodeId
    } else if (firstNode?.nodeId) {
      setSelectedNode(firstNode)
      if (firstAnchor.payload.extent?.type === 'temporal') {
        setLinkedTimestamp(firstAnchor.payload.extent?.startTimestamp)
      }
      setRefresh(!refresh)
