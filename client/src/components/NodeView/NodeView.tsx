...
import { IAnchor, INode, isSameExtent, ITemporalExtent, makeITemporalExtent, NodeIdsToNodesMap } from '../../types'
...
import { VisualisationModal } from '../Modals/VisualisationModal'
...
export const NodeView = (props: INodeViewProps) => {
  ...
  const handleStartLinkClick = () => {
    setIsLinking(true)
    if (selectedExtent === undefined || null) {
      setAlertIsOpen(true)
  ...
  //modal state for flow visualisation
  const [visualisationModalOpen, setVisualisationModalOpen] = useState(false)

  const handleVisualisationClick = useCallback(() => {
    setVisualisationModalOpen(true)
  }, [])
    ...
    <div className="node">
      <div className="nodeView" style={{ width: nodeViewWidth }}>
        <NodeHeader
          ...
          onVisualisationButtonClick={handleVisualisationClick}
        />
        <VisualisationModal
        isOpen={visualisationModalOpen}
        onClose={() => setVisualisationModalOpen(false)}
